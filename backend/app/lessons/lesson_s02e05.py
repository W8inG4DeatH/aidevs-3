...
@lesson_s02e05_bp.route("/get-mixed-files", methods=["GET"])
def get_mixed_files():
    try:
...
@lesson_s02e05_bp.route("/get-text-file-content", methods=["POST"])
...
@lesson_s02e05_bp.route("/classify-text", methods=["POST"])
def classify_text():
    data = request.json
...
@lesson_s02e05_bp.route("/transcribe-audio-file", methods=["POST"])
...
@lesson_s02e05_bp.route("/process-image", methods=["POST"])
def process_image():
    try:
        data = request.json
