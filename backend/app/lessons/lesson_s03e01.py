...
@lesson_s03e01_bp.route("/classify-text", methods=["POST"])
def classify_text():
    data = request.json
...
@lesson_s03e01_bp.route("/transcribe-audio-file", methods=["POST"])
...
@lesson_s03e01_bp.route("/process-image", methods=["POST"])
def process_image():
    try:
        data = request.json
